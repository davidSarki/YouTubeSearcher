...
- **2026-05-22** — Fixed slow startup. Three changes: (1) moved `LoadFormSettings` and TreeView state restore from `Form.Load` to `Form.Shown` in [YouTubeSearcher.vb:23-69](YouTubeSearcher.vb#L23-L69) so the window appears before the slow work runs; (2) wrapped the `LoadTreeViewState` population loop in `BeginUpdate / EndUpdate` ([DirectoryTreeViewModule.vb](DirectoryTreeViewModule.vb)) to suppress redraws during bulk node add; (3) throttled `ConvertStateToNode` progress updates from per-node to every-50-nodes — the `StatusStrip.Refresh() + Application.DoEvents()` storm on 1000+ nodes was the dominant cost. Released to `\\172.16.0.7\Administrative\Installers\forRadio\YTStatsApp`. Local `publish_stage/` folder is used to publish while the share copy is locked; gitignored.
- **2026-05-21** — Main processing flow ([`ProcessSingleMP3File`](GeneralModule.vb#L602)) now checks the MP3's `YOUTUBE_URL` custom TXXX tag before falling back to a YouTube search. New step ordering when no DB record exists by ALIAS: (1) look for `YOUTUBE_URL` tag; (2) if present and the VideoID is already in DB → mark as existing; (3) if present and new → fetch stats by VideoID and insert song + stats, skipping the YouTube search; (4) if tag absent or stats fetch fails → fall back to the original YouTube search path. New shared helper `ExtractVideoIdFromUrl` ([GeneralModule.vb](GeneralModule.vb)) handles `watch?v=`, `youtu.be/`, `embed/`, and raw 11-char IDs.
- **2026-05-21** — `CustomFileForm` auto-populates its URL column from the MP3's `YOUTUBE_URL` TXXX tag at file-add time (commit `8a17fc3`).
